...
def eval_one_epoch(hint, tgan, sampler, src, dst, ts, label):
    ...
    with torch.no_grad():
        ...
        logger.info('num of {} instances: {}'.format(hint,num_test_instance))
        logger.info('num of batches per epoch: {}'.format(num_test_batch))
...
adj_list = [[] for _ in range(max_idx + 1)] # 邻接表
...
test_rand_sampler = RandEdgeSampler(src_l, dst_l,seed=2)
nn_test_rand_sampler = RandEdgeSampler(nn_test_src_l, nn_test_dst_l,seed=3)
...
for i in range(args.n_runs):
    ...
    logger.info('num of nn_val instances: {}'.format(len(nn_val_src_l)))
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(num_batch):
            ...
            tgan = tgan.train()
            pos_prob, neg_prob = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
            # logger.info('b:{} a:{}'.format(tgan.shape_b,tgan.shape_a))
        ...
        val_acc, val_ap,val_auc = eval_one_epoch('val for old nodes', tgan, val_rand_sampler, val_src_l, 
        val_dst_l, val_ts_l, val_label_l)

        nn_val_acc, nn_val_ap, nn_val_auc = eval_one_epoch('val for new nodes', tgan, val_rand_sampler, nn_val_src_l, 
        ...
        if epoch == NUM_EPOCH - 1:
            logger.info(f'Finished,Loading the best model at epoch {early_stopper.best_epoch}')
            best_model_path = get_checkpoint_path(early_stopper.best_epoch)
            tgan.load_state_dict(torch.load(best_model_path))
            logger.info(f'Loaded the best model at epoch {early_stopper.best_epoch} for inference')
            tgan.eval()
    ...
    tgan.ngh_finder = full_ngh_finder
    test_acc, test_ap, test_auc = eval_one_epoch('test for old nodes', tgan, test_rand_sampler, test_src_l, 
    test_dst_l, test_ts_l, test_label_l)

    nn_test_acc, nn_test_ap, nn_test_auc = eval_one_epoch('test for new nodes', tgan, nn_test_rand_sampler, nn_test_src_l, 
...
logger.info('Mean Test statistics: Old nodes -- auc_mean: {}, auc_std: {}'.format(
    np.mean(test_auc_list), np.std(test_auc_list)))
logger.info('Mean Test statistics: Old nodes -- ap_mean: {}, ap_std: {}'.format(
    np.mean(test_ap_list), np.std(test_ap_list)))
logger.info('Mean Test statistics: New nodes -- auc_mean: {}, auc_std: {}'.format(
    np.mean(nn_test_auc_list), np.std(nn_test_auc_list)))
logger.info('Mean Test statistics: New nodes -- ap_mean: {}, ap_std: {}'.format(
    np.mean(nn_test_ap_list), np.std(nn_test_ap_list)))
